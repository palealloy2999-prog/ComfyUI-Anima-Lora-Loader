...
def get_lora_manager_trigger_words(name):
    try:
        import nodes as comfy_nodes

        loader_class = comfy_nodes.NODE_CLASS_MAPPINGS.get("Lora Loader (LoraManager)")
        if loader_class is None:
            logger.warning("[ANIMA LoRA] Lora Manager loader class was not found.")
            return []

        module = sys.modules.get(loader_class.__module__)
        if module is None:
            logger.warning("[ANIMA LoRA] Lora Manager module was not found in sys.modules.")
            return []

        get_info = getattr(module, "get_lora_info_absolute", None)
        format_name = getattr(module, "apply_lora_syntax_format", None)
        if not callable(get_info):
            logger.warning("[ANIMA LoRA] Lora Manager get_lora_info_absolute() was not found.")
            return []

        lora_name = format_name(name) if callable(format_name) else name
        _, trigger_words = get_info(lora_name)
        if not isinstance(trigger_words, (list, tuple)):
            return []
        return [str(word) for word in trigger_words if str(word).strip()]
    except Exception:
        logger.exception("[ANIMA LoRA] Failed to get trigger words from Lora Manager: %s", name)
        return []
    ...
    def load_loras(self, model, text, loras, new_blocks_mode=NEW_BLOCKS_MODES[0], clip=None):
        ...
        for entry in loras:
            ...
            if not entry.get("active", False):
                continue
            name = entry["name"]
            all_trigger_words.extend(get_lora_manager_trigger_words(name))
        ...
        return model, clip, trigger_words_text, " ".join(loaded), "\n".join(info)
